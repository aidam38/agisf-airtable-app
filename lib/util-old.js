...
function unparseIntervals(intervals, multiplier) {
    const unparseNumber = (n) => {
        n = n / multiplier
    ...
    return intervals.map(interval => interval.map(unparseNumber))
}

function prettyPrintCoord({ day, hour, minute }) {
...
export function prettyPrintIntervals(intervals, { increment }) {
    ...
    return unparseIntervals(intervals, multiplier).map(interval => interval.map(prettyPrintCoord).join("–")).join(", ")
}
...
function durationToHours({ hour, minute}) {
