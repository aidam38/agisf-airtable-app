...
function fitSolution(solution, config) {
    ...
    const hashes = Set(solution.map(hashCohort))
    const h = hash(hashes)
...
export async function solve(input, config, uilog) {
    ...
    do {
        ...
        uilog("Generation: " + g + ", Best fit: " + fitSolution2(currentGeneration[currentGeneration.length - 1]))
        await wait(1)
        ...
        for (let i = 0; i < currentGeneration.length; i++) {
            ...
            if (!neighbors.every(neighbor => fitSolution2(neighbor) <= fit)) {
                insertSortByKeepLengthMut(nextGeneration, n, fitSolution2, solution)
            }
